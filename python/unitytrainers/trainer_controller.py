        ...
        with tf.Session() as sess:
            ...
            try:
                while any([t.get_step <= t.get_max_steps for k, t in self.trainers.items()]) or not self.train_model:
                    ...
                    model_fwd_step_start = time.time()
                    for brain_name, trainer in self.trainers.items():
                        (take_action_vector[brain_name],
                         take_action_memories[brain_name],
                         take_action_text[brain_name],
                         take_action_outputs[brain_name]) = trainer.take_action(curr_info)
                    model_fwd_step_time = time.time() - model_fwd_step_start
                    model_fwd_step_times.append(model_fwd_step_time)
                    #print("model_fwd_step_time ", model_fwd_step_time)
                    ...
                    #print("env_step_time ", env_step_time)
                    ...
                    if global_step % self.save_freq == 0 and global_step != 0:
                        # [5:] provides warmup of 5 steps
                        model_fwd_step_time_avg = sum(model_fwd_step_times[5:])/len(model_fwd_step_times[5:])
                        env_step_time_avg = sum(env_step_times[5:])/len(env_step_times[5:])
                        print()
                        print(self.env_name)
                        print("model_fwd_step_time_avg ", model_fwd_step_time_avg)
                        print("env_step_time_avg ", env_step_time_avg)
                        print("model_fwd_step_time_avg / env_step_time_avg = ", model_fwd_step_time_avg/env_step_time_avg)
                        print()
                        end
